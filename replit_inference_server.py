...
def main():
    ...
    #model = deepspeed.init_inference(model, mp_size=world_size, dtype=torch.float, injection_policy={MPTBlock: ('attn.MultiheadAttention.Wqkv', 'ffn.down_proj')})
    ...
    model = deepspeed.init_inference(model=model, dtype=torch.float, mp_size=world_size)
    end = time.time()
    print("----------------------------------------------------")
